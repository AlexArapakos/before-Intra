...
else
{
	...
	$query="SELECT * FROM (SELECT CATALOG_NAME, QUAKEID , TO_CHAR(DATETIME,'SYYYY-MM-DD HH24:MI:SS'), LATITUDE, LONGITUDE, DEPTH, QUAKE_AGENCY, FECODE, HAS_MACRO, HAS_INFO, MAGNITUDE FROM quake_mag_catalog q WHERE q.datetime >= SYSDATE - :daysBefore AND q.source = :source ORDER BY MAGNITUDE DESC) WHERE ROWNUM <= 5";
	...
	if ($statement) 
	{		
		oci_bind_by_name($statement,":daysBefore", $daysBefore);
