...
{
	$depthString = $_GET["depthString"];
	$source = $_GET["source"];
	
	$add="";
	$depth = preg_split('/-/', $depthString);
	for($i=1; $i<count($depth); $i++)
		$add = $add."OR (DEPTH = :depth".$i.") ";
		
	$query = "SELECT CATALOG_NAME, QUAKEID , TO_CHAR(DATETIME,'SYYYY-MM-DD HH24:MI:SS'), LATITUDE, LONGITUDE, DEPTH, QUAKE_AGENCY, FECODE, HAS_MACRO, HAS_INFO, MAGNITUDE FROM quake_mag_catalog WHERE source = :source AND ((DEPTH = :depth0) ";
	$query = $query.$add.")";
	...
	if ($statement) 
	{
		...
		{
			oci_bind_by_name($statement,":depth".$i."", intval(trim($depth[$i])));
		}
